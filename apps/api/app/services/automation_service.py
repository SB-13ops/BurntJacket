...
class AutomationService:
    ...
    This gives Burnt Jacket the job contracts now. A real queue/cron worker can
    ...
    def generate_hunter_notifications(self) -> int:
        ...
        for hunt in hunts:
            results = self.db.execute(
                select(HuntResult)
                .where(HuntResult.hunt_id == hunt.id, HuntResult.burntjacket_score >= 85)
            ...
            for result in results:
                ...
                self.db.add(
                    Notification(
                        ...
                        title=f"🔥 {hunt.name}",
                        body=result.explanation or f"Burnt Jacket Score {result.burntjacket_score}",
                        payload={
                            "dedupe_key": key,
                            "hunt_id": str(hunt.id),
