"""Resolve a Burnt Jacket user from a social login profile."""
...
def find_or_create_user(db: Session, profile: SocialProfile) -> User:
    ...
    if user is None:
        user = User(
            email=(profile.email or f"{profile.provider}_{profile.subject}@users.noreply.burntjacket").lower().strip(),
